...
var (
	...
	// activated by GOVEEAM_LOG_FILE
	ApiLogFileName = "go-veeam-ent.log"

	// Globally enabling logs
	// activated by GOVEEAM_LOG
	EnableLogging = false

	// Enable logging of passwords
	// activated by GOVEEAM_LOG_PASSWORDS
	LogPasswords = false

	// Enable logging of Http requests
	// disabled by GOVEEAM_LOG_SKIP_HTTP_REQ
	LogHttpRequest = true

	// Enable logging of Http responses
	// disabled by GOVEEAM_LOG_SKIP_HTTP_RESP
...
func logSanitizedHeader(input_header http.Header) {
	for key, value := range input_header {
		if (key == "Config-Secret" || key == "authorization" || key == "Authorization" || key == "X-RestSvcSessionId") &&
			!LogPasswords {
			value = []string{"********"}
		}
		Logger.Printf("\t%s: %s\n", key, value)
